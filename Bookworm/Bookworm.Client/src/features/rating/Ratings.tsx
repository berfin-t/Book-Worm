...
export default function Rating({ bookId }: Props) {
  ...
  const [_, setLocalScore] = useState<number | null>(null);

  const myRating =
    ratings.find((r) => r.userFullName === currentUser?.name) ?? null;
  ...
  const handleUpdate = async () => {
    ...
    try {
      await dispatch(
        updateRating({ ratingId: editingId, score: editScore }),
      ).unwrap();
      setSnack({
        open: true,
    ...
    <>
      <Grid size={{ xs: 12 }}>
        ...
        <Box display="flex" flexDirection="column" gap={1}>
          ...
          {myRating ? (
            <Box display="flex" alignItems="center" gap={1}>
              <MuiRating value={myRating.score} readOnly size="small" />
              <Typography variant="body2" color="text.secondary">
                Sizin puanınız: {myRating.score} / 5
              </Typography>
              <IconButton
                size="small"
                color="primary"
            ...
            <Box display="flex" alignItems="center" gap={1}>
              <MuiRating
                ...
                onChangeActive={(_e, val) =>
                  setHoverScore(val < 1 ? null : val)
                }
                size="small"
              ...
              {status === "pendingAddRating" ? (
                ...
                <Typography variant="body2" color="text.secondary">
                  {hoverScore && hoverScore > 0
                    ? scoreLabels[hoverScore]
